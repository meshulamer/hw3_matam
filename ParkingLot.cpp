...
ParkingResult ParkingLot::enterParking(VehicleType vehicleType, LicensePlate licensePlate, Time entranceTime){
    ...
    if(CarAlreadyParked(new_parked_car, parking_array)){
        ParkingLotPrinter::printEntryFailureAlreadyParked(cout, *(parking_array[new_parked_car])->parking_spot);
    ...
    ParkingLotPrinter::printEntrySuccess(cout, new_parked_car.parking_spot);
    return SUCCESS;
}
